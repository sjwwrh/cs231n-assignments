...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    dout1, dgamma, dbeta = batchnorm_backward_alt(dout2, norm_cache)
    dx, dw, db = affine_backward(dout1, fc_cache)
    return dx, dw, db, dgamma, dbeta

def affine_layernorm_relu_forward(x, w, b, gamma, beta, ln_param):
    out1, fc_cache = affine_forward(x, w, b)
    out2, norm_cache = layernorm_forward(out1, gamma, beta, ln_param)
    out, relu_cache = relu_forward(out2)
    cache = (fc_cache, norm_cache, relu_cache)
    return out, cache
...
def affine_layernorm_relu_backward(dout, cache):
    fc_cache, norm_cache, relu_cache = cache
    dout2 = relu_backward(dout, relu_cache)
    dout1, dgamma, dbeta = layernorm_backward(dout2, norm_cache)
    dx, dw, db = affine_backward(dout1, fc_cache)
    return dx, dw, db, dgamma, dbeta
